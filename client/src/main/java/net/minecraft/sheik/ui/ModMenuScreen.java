...
import net.minecraft.sheik.ui.ModuleSettingsScreen;
...
public class ModMenuScreen extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		List<Module> modules = SheikClient.getInstance()
			.getModuleManager()
			.getModules();
		...
		for (Module module : getModulesInCategory(modules, activeCategory)) {
			...
			this.fontRendererObj.drawStringWithShadow(
				...
				listX + listWidth - 30,
				y + 5,
				module.isEnabled() ? Theme.ACCENT : Theme.TEXT_MUTED
			);
			y += ROW_STEP;
		}

		// Footer buttons — [Move HUD Modules] [Settings]
		int footerBtnGap = 6;
		int footerBtnW = (panelWidth - 40 - footerBtnGap) / 2;

		// Move HUD button (left)
		int moveHudBtnX = panelX + 20;
		boolean moveBtnHovered =
			mouseX >= moveHudBtnX &&
			mouseX <= moveHudBtnX + footerBtnW &&
			mouseY >= buttonY &&
			mouseY <= buttonY + ROW_HEIGHT;
		int btnColor = moveHudMode
			? Theme.ACCENT
			: (moveBtnHovered ? Theme.ROW_HOVER_COLOR : Theme.ROW_COLOR);
		RenderUtil.drawRoundedRect(
			moveHudBtnX,
			buttonY,
			footerBtnW,
		...
		String btnText = moveHudMode ? "Exit HUD Move" : "Move HUD Modules";
		this.drawCenteredString(
			this.fontRendererObj,
			btnText,
			moveHudBtnX + footerBtnW / 2,
			buttonY + 5,
			Theme.TEXT_PRIMARY
		);

		// Settings button (right)
		int settingsBtnX = moveHudBtnX + footerBtnW + footerBtnGap;
		boolean settingsHovered =
			mouseX >= settingsBtnX &&
			mouseX <= settingsBtnX + footerBtnW &&
			mouseY >= buttonY &&
			mouseY <= buttonY + ROW_HEIGHT;
		RenderUtil.drawRoundedRect(
			settingsBtnX,
			...
			ROW_HEIGHT,
			6,
			settingsHovered ? Theme.ROW_HOVER_COLOR : Theme.ROW_COLOR
		);
		this.drawCenteredString(
			this.fontRendererObj,
			"Settings",
			settingsBtnX + footerBtnW / 2,
			buttonY + 5,
			Theme.TEXT_PRIMARY
		);
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton)
		throws IOException {
		// Right-click on a module row → open its settings screen
		if (mouseButton == 1 && !moveHudMode) {
			List<Module> modules = SheikClient.getInstance()
				.getModuleManager()
				.getModules();
			String activeCategory = getActiveCategory(getCategories(modules));
			...
			for (Module module : getModulesInCategory(
				modules,
				activeCategory
			)) {
				if (y + ROW_HEIGHT > contentBottom) break;
				if (y < listY + 8) {
					y += ROW_STEP;
					continue;
				}
				if (
					...
					mc.displayGuiScreen(new ModuleSettingsScreen(this, module));
					return;
				}
				y += ROW_STEP;
			}
		...
		// Footer button geometry (must mirror drawScreen)
		int footerBtnGap = 6;
		int footerBtnW = (panelWidth - 40 - footerBtnGap) / 2;
		int moveHudBtnX = panelX + 20;
		int settingsBtnX = moveHudBtnX + footerBtnW + footerBtnGap;
		...
		if (
			...
			mc.displayGuiScreen(new ClientSettingScreen(this));
			return;
		}
		...
		if (moveHudMode) {
			...
			for (String category : categories) {
				...
				if (
					mouseX >= panelX + 8 &&
					mouseX <= panelX + SIDE_PANEL_WIDTH - 8 &&
					mouseY >= catY &&
					mouseY <= catY + ROW_HEIGHT
				) {
	...
	protected void mouseClickMove(
		...
		if (
			moveHudMode &&
			draggingHudModule != null &&
			draggingHudModule instanceof HudPositionable
		) {
			HudPositionable hud = (HudPositionable) draggingHudModule;
			hud.setHudX(mouseX - dragOffsetX);
			hud.setHudY(mouseY - dragOffsetY);
			SheikClient.getInstance()
				.getConfigManager()
				.save(
					SheikClient.getInstance().getModuleManager().getModules()
				);
		}
	...
	private List<Module> getModulesInCategory(
		List<Module> modules,
		String category
	) {
