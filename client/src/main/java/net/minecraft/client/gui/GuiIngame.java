...
import net.minecraft.sheik.*;
import net.minecraft.sheik.module.modules.hud.CustomCrosshair;
...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
		...
		if (
			this.mc.gameSettings.heldItemTooltips &&
		...
		if (this.recordPlayingUpFor > 0) {
			...
			if (l1 > 8) {
				...
				this.getFontRenderer().drawString(
					this.recordPlaying,
					-this.getFontRenderer().getStringWidth(this.recordPlaying) /
						2,
					-4,
					l + ((l1 << 24) & -16777216)
				);
		...
		if (this.titlesTimer > 0) {
			...
			if (i2 > 8) {
				...
				this.getFontRenderer().drawString(
					this.displayedTitle,
					(float) (-this.getFontRenderer().getStringWidth(
							this.displayedTitle
						) /
						2),
					-10.0F,
		...
		ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(
		...
		if (
			this.mc.gameSettings.keyBindPlayerList.isKeyDown() &&
		...
		if (this.mc.thePlayer.experienceLevel > 0) {
			...
			int l1 =
				(scaledRes.getScaledWidth() -
					this.getFontRenderer().getStringWidth(s)) /
				2;
	...
	public void renderSelectedItem(ScaledResolution scaledRes) {
		...
		if (
			...
			this.highlightingItemStack != null
		) {
			...
			int k = (int) (((float) this.remainingHighlightTicks * 256.0F) /
				10.0F);
	...
	public void renderDemo(ScaledResolution scaledRes) {
		...
		this.getFontRenderer().drawStringWithShadow(
			s,
			(float) (scaledRes.getScaledWidth() - i - 10),
			5.0F,
			16777215
		);
		this.mc.mcProfiler.endSection();
	...
	protected boolean showCrosshair() {
		if (
			SheikClient.getInstance()
					.getModuleManager()
					.getModule(CustomCrosshair.class) !=
				null &&
			SheikClient.getInstance()
				.getModuleManager()
				.getModule(CustomCrosshair.class)
	...
	private void renderScoreboard(
		...
		if (list.size() > 15) {
			collection = Lists.newArrayList(
				Iterables.skip(list, collection.size() - 15)
			);
		} else {
			collection = list;
		}

		int i = this.getFontRenderer().getStringWidth(
		...
		for (Score score1 : collection) {
			++j;
			ScorePlayerTeam scoreplayerteam1 = scoreboard.getPlayersTeam(
				score1.getPlayerName()
			);
			String s1 = ScorePlayerTeam.formatPlayerName(
				scoreplayerteam1,
				score1.getPlayerName()
			);
			...
			drawRect(
				...
				k,
				l,
				k + this.getFontRenderer().FONT_HEIGHT,
				1342177280
			);
			this.getFontRenderer().drawString(s1, l1, k, 553648127);
			this.getFontRenderer().drawString(
				s2,
				l - this.getFontRenderer().getStringWidth(s2),
				k,
				553648127
			);
			...
			if (j == collection.size()) {
				...
				drawRect(
					l1 - 2,
					k - this.getFontRenderer().FONT_HEIGHT - 1,
					l,
					k - 1,
					1610612736
				);
	...
	private void renderPlayerStats(ScaledResolution scaledRes) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			boolean flag =
				...
				((this.healthUpdateCounter - (long) this.updateCounter) / 3L) %
				2L ==
				1L;
			...
			if (i < this.playerHealth && entityplayer.hurtResistantTime > 0) {
				...
				i > this.playerHealth && entityplayer.hurtResistantTime > 0
			) {
				this.lastSystemTime = Minecraft.getSystemTime();
			...
			IAttributeInstance iattributeinstance =
				entityplayer.getEntityAttribute(
			...
			if (entityplayer.isPotionActive(Potion.regeneration)) {
				l2 =
					this.updateCounter % MathHelper.ceiling_float_int(f + 5.0F);
			}
			...
			for (
				int i6 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1;
				i6 >= 0;
				--i6
			) {
				int j6 = 16;
				...
				int l3 =
					MathHelper.ceiling_float_int((float) (i6 + 1) / 10.0F) - 1;
				int i4 = i1 + ((i6 % 10) * 8);
				int j4 = k1 - l3 * i2;
				...
				if (
					entityplayer.worldObj.getWorldInfo().isHardcoreModeEnabled()
				) {
					k4 = 5;
				}
			...
			if (entityplayer.isInsideOfMaterial(Material.water)) {
				...
				int k7 = MathHelper.ceiling_double_int(
					((double) (l6 - 2) * 10.0D) / 300.0D
				);
				int i8 =
					MathHelper.ceiling_double_int(
						((double) l6 * 10.0D) / 300.0D
					) -
					k7;
	...
	private void renderBossHealth() {
		if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
			...
			this.getFontRenderer().drawStringWithShadow(
				s,
				(float) (i / 2 - this.getFontRenderer().getStringWidth(s) / 2),
				(float) (i1 - 10),
				16777215
			);
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
	...
	private void renderPumpkinOverlay(ScaledResolution scaledRes) {
		...
		worldrenderer
			.pos(0.0D, (double) scaledRes.getScaledHeight(), -90.0D)
			.tex(0.0D, 1.0D)
			.endVertex();
		worldrenderer
			.pos(
				(double) scaledRes.getScaledWidth(),
				(double) scaledRes.getScaledHeight(),
				-90.0D
			)
			.tex(1.0D, 1.0D)
			.endVertex();
		worldrenderer
			.pos((double) scaledRes.getScaledWidth(), 0.0D, -90.0D)
			.tex(1.0D, 0.0D)
			.endVertex();
		...
		if (!Config.isVignetteEnabled()) {
			...
			double d0 = Math.min(
				worldborder.getResizeSpeed() *
					(double) worldborder.getWarningTime() *
					1000.0D,
				Math.abs(
					worldborder.getTargetSize() - worldborder.getDiameter()
				)
			);
	...
	private void renderPortal(float timeInPortal, ScaledResolution scaledRes) {
		...
		TextureAtlasSprite textureatlassprite =
			this.mc.getBlockRendererDispatcher()
				.getBlockModelShapes()
		...
		worldrenderer
			.pos((double) scaledRes.getScaledWidth(), 0.0D, -90.0D)
			.tex((double) f2, (double) f1)
			.endVertex();
		worldrenderer
			.pos(0.0D, 0.0D, -90.0D)
			.tex((double) f, (double) f1)
			.endVertex();
		tessellator.draw();
		GlStateManager.depthMask(true);
		GlStateManager.enableDepth();
	...
	private void renderHotbarItem(
		...
		if (itemstack != null) {
			...
			this.itemRenderer.renderItemOverlays(
		...
		if (this.mc.thePlayer != null) {
			...
			if (itemstack == null) {
				...
				(itemstack.isItemStackDamageable() ||
					...
					this.highlightingItemStack.getMetadata())
			) {
	...
	public void setRecordPlayingMessage(String recordName) {
		this.setRecordPlaying(
			I18n.format("record.nowPlaying", new Object[] { recordName }),
			true
		);
	}
	...
	public void displayTitle(
		...
		if (
			...
			this.titlesTimer =
				this.titleFadeIn + this.titleDisplayTime + this.titleFadeOut;
