...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private FloatBuffer fogColorBuffer = GLAllocation.createDirectFloatBuffer(
		16
	);
	...
	public void loadEntityShader(Entity entityIn) {
		if (OpenGlHelper.shadersSupported) {
			...
			if (entityIn instanceof EntityCreeper) {
				...
				Reflector.call(
					Reflector.ForgeHooksClient_loadEntityShader,
					new Object[] { entityIn, this }
				);
			}
		}
	}
	...
	public void activateNextShader() {
		if (
			...
			this.shaderIndex =
				(this.shaderIndex + 1) % (shaderResourceLocations.length + 1);
	...
	private void loadShader(ResourceLocation resourceLocationIn) {
		if (OpenGlHelper.isFramebufferEnabled()) {
			try {
				...
				logger.warn(
					(String) ("Failed to load shader: " + resourceLocationIn),
	...
	public void updateRenderer() {
		...
		if (this.mc.gameSettings.smoothCamera) {
			...
			float f1 = f * f * f * 8.0F;
			this.smoothCamFilterX = this.mouseFilterXAxis.smooth(
				this.smoothCamYaw,
				0.05F * f1
			);
			this.smoothCamFilterY = this.mouseFilterYAxis.smooth(
				this.smoothCamPitch,
				0.05F * f1
			);
		...
		float f2 = this.mc.theWorld.getLightBrightness(
			new BlockPos(d2, d0, d1)
		);
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			if (
				this.pointedEntity != null &&
				(d2 < d1 || this.mc.objectMouseOver == null)
			) {
				this.mc.objectMouseOver = new MovingObjectPosition(
					this.pointedEntity,
					vec33
				...
				if (
					this.pointedEntity instanceof EntityLivingBase ||
	...
	private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
		if (this.debugView) {
			...
			if (this.mc.currentScreen == null) {
				...
				flag = GameSettings.isKeyDown(
					this.mc.gameSettings.ofKeyBindZoom
				);
			}
			...
			if (
				...
				float f1 =
					(float) ((EntityLivingBase) entity).deathTime +
					partialTicks;
				f /= (1.0F - 500.0F / (f1 + 500.0F)) * 2.0F + 1.0F;
			}

			Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(
				this.mc.theWorld,
				entity,
				partialTicks
			);
			...
			return Reflector.ForgeHooksClient_getFOVModifier.exists()
				? Reflector.callFloat(
						Reflector.ForgeHooksClient_getFOVModifier,
						new Object[] {
							this,
							entity,
							block,
	...
	private void orientCamera(float partialTicks) {
		...
		double d0 =
			entity.prevPosX +
			(entity.posX - entity.prevPosX) * (double) partialTicks;
		double d1 =
			entity.prevPosY +
		...
		double d2 =
			entity.prevPosZ +
			(entity.posZ - entity.prevPosZ) * (double) partialTicks;
		...
		if (
			...
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				IBlockState iblockstate = this.mc.theWorld.getBlockState(
					blockpos
				);
				...
				if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
					...
					int j = (
						(EnumFacing) iblockstate.getValue(BlockBed.FACING)
					).getHorizontalIndex();
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				double d4 =
					(double) (-MathHelper.sin((f1 / 180.0F) * (float) Math.PI) *
						MathHelper.cos((f2 / 180.0F) * (float) Math.PI)) *
					d3;
				double d5 =
					(double) (MathHelper.cos((f1 / 180.0F) * (float) Math.PI) *
						MathHelper.cos((f2 / 180.0F) * (float) Math.PI)) *
					d3;
				double d6 =
					(double) (-MathHelper.sin(
							(f2 / 180.0F) * (float) Math.PI
						)) *
					d3;
				...
				for (int i = 0; i < 8; ++i) {
					...
					f5 = f5 * 0.1F;
					MovingObjectPosition movingobjectposition =
						this.mc.theWorld.rayTraceBlocks(
							new Vec3(
								d0 + (double) f3,
								d1 + (double) f4,
								d2 + (double) f5
					...
					if (movingobjectposition != null) {
						double d7 = movingobjectposition.hitVec.distanceTo(
							new Vec3(d0, d1, d2)
						);
		...
		if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				if (entity instanceof EntityAnimal) {
					...
					f6 =
						entityanimal1.prevRotationYawHead +
						(entityanimal1.rotationYawHead -
							entityanimal1.prevRotationYawHead) *
						partialTicks +
						180.0F;
				}

				Block block1 = ActiveRenderInfo.getBlockAtEntityViewpoint(
					this.mc.theWorld,
					entity,
					partialTicks
				);
				Object object = Reflector.newInstance(
					Reflector.EntityViewRenderEvent_CameraSetup_Constructor,
					new Object[] {
						this,
						entity,
						block1,
				...
				f8 = Reflector.getFieldValueFloat(
					object,
					Reflector.EntityViewRenderEvent_CameraSetup_roll,
					f8
				);
				f7 = Reflector.getFieldValueFloat(
					object,
					Reflector.EntityViewRenderEvent_CameraSetup_pitch,
					f7
				);
				f6 = Reflector.getFieldValueFloat(
					object,
					Reflector.EntityViewRenderEvent_CameraSetup_yaw,
					f6
				);
			...
			if (entity instanceof EntityAnimal) {
				...
				GlStateManager.rotate(
					entityanimal.prevRotationYawHead +
						(entityanimal.rotationYawHead -
							entityanimal.prevRotationYawHead) *
						partialTicks +
						180.0F,
					0.0F,
	...
	public void setupCameraTransform(float partialTicks, int pass) {
		this.farPlaneDistance =
			(float) (this.mc.gameSettings.renderDistanceChunks * 16);
		...
		if (this.cameraZoom != 1.0D) {
			GlStateManager.translate(
				(float) this.cameraYaw,
				(float) (-this.cameraPitch),
				0.0F
			);
		...
		float f1 =
			...
			(this.mc.thePlayer.timeInPortal -
				this.mc.thePlayer.prevTimeInPortal) *
			partialTicks;
	...
	public void renderHand(
		float p_renderHand_1_,
		int p_renderHand_2_,
		boolean p_renderHand_3_,
		boolean p_renderHand_4_,
		boolean p_renderHand_5_
	) {
		if (!this.debugView) {
			GlStateManager.matrixMode(5889);
			GlStateManager.loadIdentity();
			...
			Project.gluPerspective(
				this.getFOVModifier(p_renderHand_1_, false),
				(float) this.mc.displayWidth / (float) this.mc.displayHeight,
				0.05F,
			...
			if (this.mc.gameSettings.anaglyph) {
				GlStateManager.translate(
					(float) (p_renderHand_2_ * 2 - 1) * 0.1F,
					0.0F,
					0.0F
				);
			}
			...
			if (p_renderHand_3_) {
				...
				boolean flag1 = !ReflectorForge.renderFirstPersonHand(
	...
	private void updateLightmap(float partialTicks) {
		if (this.lightmapUpdateNeeded) {
			...
			if (world != null) {
				...
				for (int i = 0; i < 256; ++i) {
					float f2 =
						world.provider.getLightBrightnessTable()[i / 16] * f1;
					float f3 =
						world.provider.getLightBrightnessTable()[i % 16] *
					...
					if (this.mc.thePlayer.isPotionActive(Potion.nightVision)) {
						float f15 = this.getNightVisionBrightness(
							this.mc.thePlayer,
							partialTicks
						);
						float f12 = 1.0F / f8;
	...
	public float getNightVisionBrightness(
		...
		return i > 200
			...
			: 0.7F +
				MathHelper.sin(
					((float) i - partialTicks) * (float) Math.PI * 0.2F
				) *
				0.3F;
	}
	...
	public void updateCameraAndRender(float partialTicks, long nanoTime) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.theWorld != null) {
				...
				if (
					...
					if (
						this.mc.gameSettings.ofShowFps &&
				...
				TileEntityRendererDispatcher.instance.renderEngine =
					this.mc.getTextureManager();
				TileEntityRendererDispatcher.instance.fontRenderer =
			...
			if (this.mc.currentScreen != null) {
				...
				try {
					if (Reflector.ForgeHooksClient_drawScreen.exists()) {
						Reflector.callVoid(
							Reflector.ForgeHooksClient_drawScreen,
							new Object[] {
							...
							public String call() throws Exception {
								return String.format(
									...
									new Object[] {
										Integer.valueOf(k1),
										Integer.valueOf(l1),
										Integer.valueOf(Mouse.getX()),
										Integer.valueOf(Mouse.getY()),
									}
								);
							}
						}
		...
		SheikClient.getInstance().onRender2D(partialTicks);
		this.frameFinish();
	...
	private boolean isDrawBlockOutline() {
		if (!this.drawBlockOutline) {
			...
			Entity entity = this.mc.getRenderViewEntity();
			boolean flag =
				entity instanceof EntityPlayer && !this.mc.gameSettings.hideGUI;
			...
			if (flag && !((EntityPlayer) entity).capabilities.allowEdit) {
				...
				if (
					...
					this.mc.objectMouseOver.typeOfHit ==
					...
					if (
						this.mc.playerController.getCurrentGameType() ==
						WorldSettings.GameType.SPECTATOR
					) {
						flag =
							ReflectorForge.blockHasTileEntity(iblockstate) &&
							this.mc.theWorld.getTileEntity(blockpos) instanceof
								IInventory;
					} else {
						flag =
							itemstack != null &&
							(itemstack.canDestroy(block) ||
	...
	private void renderWorldDirections(float partialTicks) {
		if (
			...
			RenderGlobal.drawOutlinedBoundingBox(
				new AxisAlignedBB(0.0D, 0.0D, 0.0D, 0.005D, 1.0E-4D, 1.0E-4D),
				255,
				0,
				0,
				255
			...
			RenderGlobal.drawOutlinedBoundingBox(
				new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0E-4D, 1.0E-4D, 0.005D),
				0,
				0,
				255,
				255
			...
			RenderGlobal.drawOutlinedBoundingBox(
				new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0E-4D, 0.0033D, 1.0E-4D),
				0,
				255,
				0,
				255
	...
	private void renderWorldPass(
		...
		clippinghelper.disabled =
			Config.isShaders() && !Shaders.isFrustumCulling();
		...
		if (
			...
			128.0D + (double) (this.mc.gameSettings.ofCloudsHeight * 128.0F)
		) {
		...
		this.mc.getTextureManager()
			...
			.setBlurMipmap(false, this.mc.gameSettings.mipmapLevels > 0);
		renderglobal.renderBlockLayer(
		...
		if (
			Reflector.ForgeHooksClient_setRenderPass.exists() && !this.debugView
		) {
			RenderHelper.enableStandardItemLighting();
			this.mc.mcProfiler.endStartSection("entities");
			Reflector.callVoid(
	...
	private void renderCloudsCheck(
		RenderGlobal renderGlobalIn,
		float partialTicks,
		int pass
	) {
		if (
			this.mc.gameSettings.renderDistanceChunks >= 4 &&
			!Config.isCloudsOff() &&
			Shaders.shouldRenderClouds(this.mc.gameSettings)
		) {
		...
		if (f != 0.0F && Config.isRainSplash()) {
			...
			for (int l = 0; l < k; ++l) {
				BlockPos blockpos1 = world.getPrecipitationHeight(
					blockpos.add(
						this.random.nextInt(i) - this.random.nextInt(i),
						0,
						this.random.nextInt(i) - this.random.nextInt(i)
					)
				);
				BiomeGenBase biomegenbase = world.getBiomeGenForCoords(
					blockpos1
				);
				...
				if (
					...
					if (block.getMaterial() == Material.lava) {
						this.mc.theWorld.spawnParticle(
						...
						if (this.random.nextInt(j) == 0) {
							...
							d1 =
								(double) ((float) blockpos2.getY() + 0.1F) +
			...
			if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
				...
				if (
					d1 > (double) (blockpos.getY() + 1) &&
					world.getPrecipitationHeight(blockpos).getY() >
					MathHelper.floor_float((float) blockpos.getY())
				) {
	...
	protected void renderRainSnow(float partialTicks) {
		if (Reflector.ForgeWorldProvider_getWeatherRenderer.exists()) {
			...
			if (object != null) {
				Reflector.callVoid(
					object,
					Reflector.IRenderHandler_render,
					new Object[] {
						Float.valueOf(partialTicks),
						this.mc.theWorld,
						this.mc,
		...
		if (f5 > 0.0F) {
			...
			for (int k1 = k - i1; k1 <= k + i1; ++k1) {
				for (int l1 = i - i1; l1 <= i + i1; ++l1) {
					...
					if (
						biomegenbase.canRain() || biomegenbase.getEnableSnow()
					) {
						int j2 = world
							.getPrecipitationHeight(blockpos$mutableblockpos)
							.getY();
						int k2 = j - i1;
						int l2 = j + i1;
						...
						if (k2 != l2) {
							this.random.setSeed(
								(long) ((l1 * l1 * 3121 + l1 * 45238971) ^
									((k1 * k1 * 418711) + k1 * 13761))
							);
							blockpos$mutableblockpos.set(l1, k2, k1);
							float f1 = biomegenbase.getFloatTemperature(
								blockpos$mutableblockpos
							);
							...
							if (
								...
								double d5 =
									(((double) ((this.rendererUpdateCount +
													l1 * l1 * 3121 +
													l1 * 45238971 +
													k1 * k1 * 418711 +
													k1 * 13761) &
								...
								double d6 =
									(double) ((float) l1 + 0.5F) - entity.posX;
								double d7 =
									(double) ((float) k1 + 0.5F) - entity.posZ;
								float f2 =
									MathHelper.sqrt_double(d6 * d6 + d7 * d7) /
									(float) i1;
								float f3 =
									((1.0F - f2 * f2) * 0.5F + 0.5F) * f5;
								blockpos$mutableblockpos.set(l1, i3, k1);
								int j3 = world.getCombinedLight(
									blockpos$mutableblockpos,
									0
								);
								int k3 = (j3 >> 16) & 65535;
								...
								double d8 =
									(double) (((float) (this.rendererUpdateCount &
												511) +
											partialTicks) /
										512.0F);
								double d9 =
									...
									(double) f *
									0.01D *
									(double) ((float) this.random.nextGaussian());
								double d10 =
									this.random.nextDouble() +
									(double) (f *
								...
								float f6 =
									...
									(float) i1;
								float f4 =
									((1.0F - f6 * f6) * 0.3F + 0.5F) * f5;
								blockpos$mutableblockpos.set(l1, i3, k1);
								int i4 =
									(world.getCombinedLight(
												blockpos$mutableblockpos,
												0
											) *
											3 +
	...
	private void updateFogColor(float partialTicks) {
		...
		float f =
			0.25F +
			(0.75F * (float) this.mc.gameSettings.renderDistanceChunks) / 32.0F;
		f = 1.0F - (float) Math.pow((double) f, 0.25D);
		Vec3 vec3 = world.getSkyColor(
			this.mc.getRenderViewEntity(),
			partialTicks
		);
		vec3 = CustomColors.getWorldSkyColor(
			vec3,
			world,
			this.mc.getRenderViewEntity(),
			partialTicks
		...
		vec31 = CustomColors.getWorldFogColor(
			vec31,
			world,
			this.mc.getRenderViewEntity(),
			partialTicks
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			double d0 = -1.0D;
			Vec3 vec32 =
				MathHelper.sin(world.getCelestialAngleRadians(partialTicks)) >
				0.0F
			...
			if (f5 > 0.0F) {
				float[] afloat = world.provider.calcSunriseSunsetColors(
					world.getCelestialAngle(partialTicks),
					partialTicks
				);
				...
				if (afloat != null) {
					f5 = f5 * afloat[3];
					this.fogColorRed =
						this.fogColorRed * (1.0F - f5) + afloat[0] * f5;
					this.fogColorGreen =
						this.fogColorGreen * (1.0F - f5) + afloat[1] * f5;
					this.fogColorBlue =
						this.fogColorBlue * (1.0F - f5) + afloat[2] * f5;
				}
			}
		}
		...
		if (this.cloudFog) {
			...
			if (
				entity instanceof EntityLivingBase &&
				((EntityLivingBase) entity).isPotionActive(
			...
			Vec3 vec35 = CustomColors.getUnderwaterColor(
				this.mc.theWorld,
				this.mc.getRenderViewEntity().posX,
				this.mc.getRenderViewEntity().posY + 1.0D,
			...
			Vec3 vec34 = CustomColors.getUnderlavaColor(
				this.mc.theWorld,
				this.mc.getRenderViewEntity().posX,
				this.mc.getRenderViewEntity().posY + 1.0D,
				this.mc.getRenderViewEntity().posZ
		...
		float f13 =
			this.fogColor2 + (this.fogColor1 - this.fogColor2) * partialTicks;
		...
		if (
			entity instanceof EntityLivingBase &&
			((EntityLivingBase) entity).isPotionActive(Potion.blindness)
		) {
			int i = ((EntityLivingBase) entity).getActivePotionEffect(
				Potion.blindness
			).getDuration();
		...
		if (this.bossColorModifier > 0.0F) {
			float f14 =
				this.bossColorModifierPrev +
				(this.bossColorModifier - this.bossColorModifierPrev) *
				partialTicks;
			this.fogColorRed =
				this.fogColorRed * (1.0F - f14) + this.fogColorRed * 0.7F * f14;
			this.fogColorGreen =
				this.fogColorGreen * (1.0F - f14) +
				this.fogColorGreen * 0.6F * f14;
			this.fogColorBlue =
				this.fogColorBlue * (1.0F - f14) +
				this.fogColorBlue * 0.6F * f14;
		}

		if (
			entity instanceof EntityLivingBase &&
			((EntityLivingBase) entity).isPotionActive(Potion.nightVision)
		) {
			float f15 = this.getNightVisionBrightness(
			...
			this.fogColorRed =
				this.fogColorRed * (1.0F - f15) + this.fogColorRed * f6 * f15;
			this.fogColorGreen =
				this.fogColorGreen * (1.0F - f15) +
				this.fogColorGreen * f6 * f15;
			this.fogColorBlue =
				this.fogColorBlue * (1.0F - f15) + this.fogColorBlue * f6 * f15;
		}

		if (this.mc.gameSettings.anaglyph) {
			float f16 =
				(this.fogColorRed * 30.0F +
					this.fogColorGreen * 59.0F +
					this.fogColorBlue * 11.0F) /
				100.0F;
			float f17 =
				(this.fogColorRed * 30.0F + this.fogColorGreen * 70.0F) /
				100.0F;
			float f7 =
				(this.fogColorRed * 30.0F + this.fogColorBlue * 70.0F) / 100.0F;
		...
		if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
			Object object = Reflector.newInstance(
				...
				new Object[] {
					...
					block,
					Float.valueOf(partialTicks),
					Float.valueOf(this.fogColorRed),
					Float.valueOf(this.fogColorGreen),
					Float.valueOf(this.fogColorBlue),
				}
			);
			Reflector.postForgeBusEvent(object);
			this.fogColorRed = Reflector.getFieldValueFloat(
				object,
				Reflector.EntityViewRenderEvent_FogColors_red,
				this.fogColorRed
			);
			this.fogColorGreen = Reflector.getFieldValueFloat(
				object,
				Reflector.EntityViewRenderEvent_FogColors_green,
				this.fogColorGreen
			);
			this.fogColorBlue = Reflector.getFieldValueFloat(
				object,
	...
	private void setupFog(int startCoords, float partialTicks) {
		...
		GL11.glFog(
			GL11.GL_FOG_COLOR,
			(FloatBuffer) this.setFogColorBuffer(
				this.fogColorRed,
				this.fogColorGreen,
				this.fogColorBlue,
				1.0F
		...
		if (Reflector.ForgeHooksClient_getFogDensity.exists()) {
			f = Reflector.callFloat(
				Reflector.ForgeHooksClient_getFogDensity,
				new Object[] {
					this,
					entity,
					block,
		...
		if (f >= 0.0F) {
			...
			if (i < 20) {
				f4 =
					5.0F +
					(this.farPlaneDistance - 5.0F) * (1.0F - (float) i / 20.0F);
			}
			...
			if (
				...
				float f2 =
					0.1F -
					(float) EnchantmentHelper.getRespiration(entity) * 0.03F;
			...
			if (
				this.mc.theWorld.provider.doesXZShowFog(
					(int) entity.posX,
					(int) entity.posZ
				)
			) {
	...
	private FloatBuffer setFogColorBuffer(
		float red,
		float green,
		float blue,
		float alpha
		...
		if (Config.isSmoothWorld() && Config.isSingleProcessor()) {
			if (this.mc.isIntegratedServerRunning()) {
				...
				if (integratedserver != null) {
					...
					if (
						!flag &&
						!(this.mc.currentScreen instanceof GuiDownloadTerrain)
					) {
						...
						if (
							...
							this.lastServerTicks =
								integratedserver.getTickCounter();
	...
	private void frameInit() {
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				String s = "HD_U".replace("HD_U", "HD Ultra").replace(
					"L",
					"Light"
				);
		...
		if (
			this.mc.currentScreen != null &&
			this.mc.currentScreen.getClass() == GuiChat.class
		) {
			this.mc.displayGuiScreen(
				new GuiChatOF((GuiChat) this.mc.currentScreen)
			);
		}
	}
	...
	private void updateMainMenu(GuiMainMenu p_updateMainMenu_1_) {
		try {
			...
			Reflector.setFieldValue(
				p_updateMainMenu_1_,
				Reflector.GuiMainMenu_splashText,
				s
			);
	...
	public boolean setFxaaShader(int p_setFxaaShader_1_) {
		if (!OpenGlHelper.isFramebufferEnabled()) {
			...
			this.theShaderGroup != null &&
			this.theShaderGroup == this.fxaaShaders[p_setFxaaShader_1_]
		) {
			return true;
	...
	private void checkLoadVisibleChunks(
		Entity p_checkLoadVisibleChunks_1_,
		float p_checkLoadVisibleChunks_2_,
		ICamera p_checkLoadVisibleChunks_3_,
		boolean p_checkLoadVisibleChunks_4_
	) {
		int i = 201435902;
		...
		if (this.loadVisibleChunks) {
			...
			this.loadAllVisibleChunks(
				p_checkLoadVisibleChunks_1_,
				(double) p_checkLoadVisibleChunks_2_,
				p_checkLoadVisibleChunks_3_,
				p_checkLoadVisibleChunks_4_
			);
		...
		if (Keyboard.isKeyDown(61) && Keyboard.isKeyDown(38)) {
			...
			ChatComponentText chatcomponenttext = new ChatComponentText(
				I18n.format("of.message.loadingVisibleChunks", new Object[0])
			);
	...
	private void loadAllVisibleChunks(
		Entity p_loadAllVisibleChunks_1_,
		double p_loadAllVisibleChunks_2_,
		ICamera p_loadAllVisibleChunks_4_,
		boolean p_loadAllVisibleChunks_5_
	) {
		...
		try {
			...
			while (true) {
				...
				for (int j1 = 0; j1 < 100; ++j1) {
					...
					renderglobal.setupTerrain(
						p_loadAllVisibleChunks_1_,
						p_loadAllVisibleChunks_2_,
						p_loadAllVisibleChunks_4_,
						this.frameCount++,
						p_loadAllVisibleChunks_5_
					);
