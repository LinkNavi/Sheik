...
DebugMenu::DebugMenu(QWidget *parent) : QMainWindow(parent) {
  ...
  addRow(6, "Held Item Type", lblHeldItemTypeVal);
  ...
  // ── Connections ──────────────────────────────────────────────
  connect(btnBack, &QPushButton::clicked, this, [this]() { emit goBack(); });
...
void DebugMenu::updateLoop() {
  ...
  lblTargetVal->setText(s->targetId != -1 ? QString("#%1").arg(s->targetId)
                                          : "none");
  ...
  switch (ipc.heldItemType()) {
  case HeldItem::NONE:
    lblHeldItemTypeVal->setText("None");
    break;
  case HeldItem::BLOCK:
    lblHeldItemTypeVal->setText("Block");
    break;
  case HeldItem::SWORD:
    lblHeldItemTypeVal->setText("Sword");
    break;
  case HeldItem::AXE:
    lblHeldItemTypeVal->setText("Axe");
    break;
  }
  float hp = s->targetHealth;
  lblHealthVal->setText(
      s->targetId != -1 ? QString("%1 / 20").arg(hp, 0, 'f', 1) : "—");
  hpBar->setValue(s->targetId != -1 ? static_cast<int>(hp) : 0);
}
